...
async function compileSFCTemplate(code, id) {
        ...
        (node) => {
          if (node.type === 1) {
            if (node.tagType === 0 && !EXCLUDE_TAG.includes(node.tag)) {
              ...
              !node.loc.source.includes("data-v-inspector-file") && s.prependLeft(node.loc.start.offset + node.tag.length + 1, ` data-v-inspector-file="${id}" data-v-inspector-line=${node.loc.start.line} data-v-inspector-column=${node.loc.start.column} data-v-inspector-title="${base}"`);
            }
          }
        }
      ]
...
function normalizeOverlayScripts({ hash, scripts }) {
  const s = new import_magic_string2.default(scripts);
...
var v2 = {
  scripts: `import Vue from "/node_modules/.vite/vue.js?v=browserHash"
...
function guessEditor() {
  ...
  try {
    if (process.platform === "darwin") {
      ...
      const output = import_child_process.default.execSync('wmic process where "executablepath is not null" get executablepath').toString();
...
// src/index.ts
function VitePluginInspector(options = { vue: 3 }) {
      ...
      return {
        ...
        tags: [{
          ...
          children: normalizeOverlayScripts({
            hash: server._optimizeDepsMetadata.browserHash,
            scripts
          }),
          injectTo: "body"
        }, {
